...
const PRIMARY = "#FFD300";

const KATEGORI_STYLE: Record<string, { bg: string; color: string }> = {
  "Outstanding":      { bg: "rgba(167,139,250,0.15)", color: "#A78BFA" },
  "Sangat Baik":      { bg: "rgba(34,197,94,0.15)",   color: "#22C55E" },
  "Baik":             { bg: "rgba(59,130,246,0.15)",  color: "#3B82F6" },
  "Cukup":            { bg: "rgba(255,211,0,0.15)",   color: "#FFD300" },
  "Perlu Pembinaan":  { bg: "rgba(245,158,11,0.15)",  color: "#F59E0B" },
  "Evaluasi Khusus":  { bg: "rgba(239,68,68,0.15)",   color: "#EF4444" },
};
...
function Card({ label, value, sub, accent }: { label: string; value: string | number; sub?: string; accent?: boolean }) {
  return (
    <div className="rounded-xl p-4" style={{
      background: accent ? "rgba(255,211,0,0.08)" : "var(--bg-card)",
      border: accent ? "1px solid rgba(255,211,0,0.3)" : "1px solid var(--border)",
    }}>
      <p className="text-2xl font-black" style={{ color: accent ? "#FFD300" : "var(--text-primary)" }}>{value}</p>
      <p className="text-[11px] font-bold uppercase tracking-wide mt-0.5" style={{ color: "var(--text-muted)" }}>{label}</p>
      {sub && <p className="text-[11px] mt-0.5 truncate" style={{ color: "var(--text-secondary)" }}>{sub}</p>}
    </div>
  );
}

function KatBadge({ kat }: { kat: string }) {
  const style = KATEGORI_STYLE[kat] ?? { bg: "rgba(75,85,99,0.15)", color: "#6B7280" };
  return (
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-black" style={{ color: "var(--text-primary)" }}>KPI Staff Bandara</h1>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {canInput && (
            <button onClick={openNew}
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl font-bold text-sm text-black hover:opacity-90 transition-colors"
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <input type="number" value={filterTahun} min={2024} max={2099}
          ...
          className="w-24 text-sm rounded-xl px-3 py-2 focus:outline-none"
          style={{ background: "var(--bg-card)", border: "1px solid var(--border)", color: "var(--text-primary)" }} />
      </div>
      ...
      {activeTab === "nasional" && (
        <div className="rounded-xl overflow-hidden" style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }}>
                ...
                {curPeriod.map((r, i) => {
                  ...
                  return (
                    <tr key={r.id} className="hover:bg-white/3 transition-colors">
                      <td className="px-4 py-3 font-black" style={{ color: "var(--text-muted)" }}>
                        {i===0?"🥇":i===1?"🥈":i===2?"🥉":`#${i+1}`}
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <div className="w-8 h-8 rounded-lg flex-shrink-0 flex items-center justify-center font-black text-xs text-black"
                            style={{ background: PRIMARY }}>
                            {st?.nama?.[0]?.toUpperCase() ?? "S"}
                          </div>
                          <div>
                            <p className="font-semibold whitespace-nowrap" style={{ color: "var(--text-primary)" }}>{st?.nama ?? "—"}</p>
                            <p className="text-xs" style={{ color: "var(--text-muted)" }}>{st?.staff_code} · {st?.jabatan}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <p className="font-bold" style={{ color: "var(--text-primary)" }}>{(r.nilai_kpi_saldo??0).toFixed(1)}</p>
                        <p className="text-[11px]" style={{ color: "var(--text-muted)" }}>{(r.persen_saldo??0).toFixed(0)}% saldo</p>
                        <ScoreBar value={r.persen_saldo??0} max={120} />
                      </td>
                      <td className="px-4 py-3">
                        <p className="font-bold" style={{ color: "var(--text-primary)" }}>{(r.nilai_kpi_pembinaan??0).toFixed(1)}</p>
                        <p className="text-[11px]" style={{ color: "var(--text-muted)" }}>{r.driver_dibina} driver</p>
                        <ScoreBar value={r.persen_pembinaan??0} color="#3B82F6" />
                      </td>
                      <td className="px-4 py-3">
                        <p className="font-bold" style={{ color: "var(--text-primary)" }}>{(r.nilai_kpi_performa??0).toFixed(1)}</p>
                        <p className="text-[11px]" style={{ color: "var(--text-muted)" }}>Abs {r.skor_absensi?.toFixed(0)}% / Pel {r.skor_pelayanan?.toFixed(0)}</p>
                      ...
                      <td className="px-4 py-3">
                        {canInput && (
                          <button onClick={() => openEdit(r)} className="p-1.5 rounded-lg transition-colors hover:bg-white/10"
                ...
                {curPeriod.length === 0 && (
                  <tr>
                    <td colSpan={10} className="py-16 text-center">
                      ...
                      <p className="font-semibold" style={{ color: "var(--text-muted)" }}>Belum ada data KPI untuk {BULAN[filterBulan]} {filterTahun}</p>
                      {canInput && <p className="text-sm mt-1" style={{ color: "var(--text-muted)" }}>Klik "Input KPI" untuk mulai</p>}
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="px-5 py-3 border-t text-xs font-medium" style={{ borderColor: "var(--border)", color: "var(--text-muted)" }}>
      ...
      {activeTab === "cabang" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {airports.map(ap => {
            ...
            return (
              <div key={ap.id} className="rounded-xl overflow-hidden"
                style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderTop: `3px solid ${PRIMARY}` }}>
                <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: "var(--border)" }}>
                  <div>
                    <p className="font-black" style={{ color: "var(--text-primary)" }}>{ap.city}</p>
                    <p className="text-xs" style={{ color: "var(--text-muted)" }}>{ap.code} · {staffCountByAirport[ap.id]??0} staff · {driverCountByAirport[ap.id]??0} driver</p>
                  </div>
                  <div className="text-right">
                    <p className="text-xl font-black" style={{ color: avg >= 90 ? "#22C55E" : avg >= 70 ? "#F59E0B" : "#EF4444" }}>
                      {recs.length ? avg.toFixed(1) : "—"}
                    </p>
                    <p className="text-xs" style={{ color: "var(--text-muted)" }}>avg KPI</p>
                  </div>
                </div>
                {recs.length === 0 ? (
                  <div className="px-5 py-8 text-center text-sm" style={{ color: "var(--text-muted)" }}>Belum ada input KPI</div>
                ) : (
                    ...
                    {recs.map(r => {
                      ...
                      return (
                        <div key={r.id} className="flex items-center justify-between px-5 py-3">
                          <div>
                            <p className="font-semibold text-sm" style={{ color: "var(--text-primary)" }}>{st?.nama ?? "—"}</p>
                            <p className="text-xs" style={{ color: "var(--text-muted)" }}>{st?.jabatan}</p>
                          </div>
                          <div className="flex items-center gap-3">
                            <KatBadge kat={r.kategori ?? kategoriOf(r.kpi_akhir??0)} />
                            <span className="font-black w-12 text-right" style={{ color: "var(--text-primary)" }}>{(r.kpi_akhir??0).toFixed(1)}</span>
                          </div>
                        </div>
                      );
                    })}
        ...
        <div className="rounded-xl overflow-hidden" style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }}>
          {perluPembinaan.length === 0 ? (
            <div className="py-16 text-center">
              <p className="text-3xl mb-2">🎉</p>
              <p className="font-semibold" style={{ color: "var(--text-muted)" }}>Tidak ada staff dengan KPI &lt; 70 bulan ini</p>
            </div>
          ) : (
            <>
              <div className="px-5 py-4 border-b" style={{ borderColor: "var(--border)", background: "rgba(239,68,68,0.08)" }}>
                  ...
                  return (
                    <div key={r.id} className="flex items-center gap-4 px-5 py-4">
                      ...
                      <div className="flex-1">
                        <p className="font-semibold" style={{ color: "var(--text-primary)" }}>{st?.nama ?? "—"}</p>
                        <p className="text-xs" style={{ color: "var(--text-muted)" }}>{ap?.city} · {st?.jabatan}</p>
                      ...
                      <div className="text-right">
                        <p className="text-2xl font-black" style={{ color: "#EF4444" }}>{(r.kpi_akhir??0).toFixed(1)}</p>
                        <KatBadge kat={r.kategori ?? "Evaluasi Khusus"} />
                      </div>
                      {canInput && (
                        <button onClick={() => openEdit(r)} className="p-2 rounded-xl transition-colors hover:bg-white/10"
                          style={{ color: "var(--text-muted)" }}>
                          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-4 h-4">
                            <path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/>
                          </svg>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setShowModal(false)}>
          <div className="rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}
            style={{ background: "var(--bg-card)", border: "1px solid var(--border-strong)", borderTop: `3px solid ${PRIMARY}` }}>

            {/* Modal header */}
            <div className="flex items-center justify-between px-6 py-5 sticky top-0 z-10 border-b rounded-t-2xl"
              style={{ background: "var(--bg-card)", borderColor: "var(--border)" }}>
              <div>
                <h2 className="font-black" style={{ color: "var(--text-primary)" }}>{editId ? "Edit KPI Staff" : "Input KPI Staff"}</h2>
                <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>{BULAN[filterBulan]} {filterTahun}</p>
              </div>
              <button onClick={() => setShowModal(false)} className="p-2 rounded-xl hover:bg-white/10" style={{ color: "var(--text-muted)" }}>
            ...
            <div className="p-6 space-y-5">
              {/* Staff selector */}
              <div>
                <label className="text-xs font-bold uppercase tracking-wide mb-1 block" style={{ color: "var(--text-muted)" }}>Staff</label>
                ...
                {selAirport && (
                  <div className="mt-2 flex gap-3 text-xs rounded-xl px-3 py-2" style={{ background: "var(--bg-elevated)", color: "var(--text-muted)" }}>
                    <span>Cabang: <strong style={{ color: "var(--text-primary)" }}>{selAirport.city}</strong></span>
                    <span>Driver: <strong style={{ color: "var(--text-primary)" }}>{selDriverAktif}</strong></span>
                    <span>Staff: <strong style={{ color: "var(--text-primary)" }}>{selStaffAktif}</strong></span>
                  </div>
                )}
              </div>

              {/* Modul 1 */}
              <div className="rounded-xl p-4 space-y-3" style={{ background: "rgba(255,211,0,0.06)", border: "1px solid rgba(255,211,0,0.2)" }}>
                <p className="text-sm font-bold flex items-center gap-2" style={{ color: "#FFD300" }}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs font-semibold mb-1 block" style={{ color: "var(--text-muted)" }}>Target Saldo (Rp)</label>
                  ...
                  <div>
                    <label className="text-xs font-semibold mb-1 block" style={{ color: "var(--text-muted)" }}>Realisasi Saldo (Rp)</label>
                ...
                <div className="flex gap-4 text-sm">
                  <span style={{ color: "var(--text-muted)" }}>Persentase: <strong style={{ color: "var(--text-primary)" }}>{prev.ps.toFixed(1)}%</strong></span>
                  <span style={{ color: "var(--text-muted)" }}>Nilai: <strong style={{ color: "#FFD300" }}>{prev.ns.toFixed(1)} poin</strong></span>
                  {prev.bon > 0 && <span className="font-bold" style={{ color: "#22C55E" }}>+{prev.bon.toFixed(1)} bonus</span>}
                </div>
              </div>

              {/* Modul 2 */}
              <div className="rounded-xl p-4 space-y-3" style={{ background: "rgba(59,130,246,0.06)", border: "1px solid rgba(59,130,246,0.2)" }}>
                <p className="text-sm font-bold flex items-center gap-2" style={{ color: "#3B82F6" }}>
                  <span className="w-5 h-5 rounded-lg text-white text-xs font-black flex items-center justify-center" style={{ background: "#3B82F6" }}>2</span>
                  Pembinaan Driver — 30%
                </p>
                <div className="rounded-xl px-3 py-2 text-xs space-y-0.5" style={{ background: "rgba(59,130,246,0.1)", color: "#3B82F6" }}>
                ...
                <div>
                  <label className="text-xs font-semibold mb-1 block" style={{ color: "var(--text-muted)" }}>Driver Dibina oleh Staff ini</label>
                ...
                <div className="flex gap-4 text-sm">
                  <span style={{ color: "var(--text-muted)" }}>Persentase: <strong style={{ color: "var(--text-primary)" }}>{prev.pp.toFixed(1)}%</strong></span>
                  <span style={{ color: "var(--text-muted)" }}>Nilai: <strong style={{ color: "#3B82F6" }}>{prev.np.toFixed(1)} poin</strong></span>
                </div>
              </div>

              {/* Modul 3 */}
              <div className="rounded-xl p-4 space-y-3" style={{ background: "rgba(34,197,94,0.06)", border: "1px solid rgba(34,197,94,0.2)" }}>
                <p className="text-sm font-bold flex items-center gap-2" style={{ color: "#22C55E" }}>
                  <span className="w-5 h-5 rounded-lg text-white text-xs font-black flex items-center justify-center" style={{ background: "#22C55E" }}>3</span>
                  Performa Staff — 20%
                </p>
                <div className="rounded-xl px-3 py-2.5 flex items-center justify-between" style={{ background: "rgba(34,197,94,0.08)" }}>
                  <div>
                    <p className="text-xs font-semibold" style={{ color: "#22C55E" }}>A. Absensi (40%) — OTOMATIS</p>
                    <p className="text-[11px] mt-0.5" style={{ color: "var(--text-muted)" }}>Dari data kehadiran & jadwal kerja</p>
                  </div>
                ...
                <div className="space-y-2">
                  <div>
                    <label className="text-xs font-semibold mb-1 block" style={{ color: "var(--text-muted)" }}>B. Pelayanan Penumpang (40%) — 0–100</label>
                    <div className="flex items-center gap-3">
                      <input type="number" min="0" max="100" value={form.skor_pelayanan || ""}
                        ...
                        className="w-24 rounded-xl px-3 py-2 text-sm text-center focus:outline-none"
                        style={{ background: "var(--bg-elevated)", border: "1px solid var(--border)", color: "var(--text-primary)" }} />
                      <ScoreBar value={form.skor_pelayanan} color="#10B981" />
                    </div>
                  </div>
                  <div>
                    <label className="text-xs font-semibold mb-1 block" style={{ color: "var(--text-muted)" }}>C. Kerapian & Kebersihan (20%) — 0–100</label>
                    <div className="flex items-center gap-3">
                ...
                <div className="text-sm">
                  <span style={{ color: "var(--text-muted)" }}>Nilai: <strong style={{ color: "#22C55E" }}>{prev.nkp.toFixed(1)} poin</strong></span>
                </div>
              </div>

              {/* KPI Preview */}
              <div className="rounded-xl p-4 flex items-center justify-between"
                style={{ background: "rgba(255,211,0,0.08)", border: "2px solid rgba(255,211,0,0.35)" }}>
                <div>
                  <p className="text-xs font-black uppercase tracking-widest" style={{ color: "#FFD300" }}>KPI Akhir</p>
                  <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>
                    {prev.ns.toFixed(1)} + {prev.np.toFixed(1)} + {prev.nkp.toFixed(1)} + {prev.bon.toFixed(1)} bonus
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-4xl font-black" style={{ color: "#FFD300" }}>{prev.akhir.toFixed(1)}</p>
              ...
              {/* Catatan */}
              <div>
                <label className="text-xs font-bold uppercase tracking-wide mb-1 block" style={{ color: "var(--text-muted)" }}>Catatan (opsional)</label>
              ...
              <div className="flex gap-3">
                ...
                <button onClick={handleSave} disabled={saving}
                  className="flex-1 py-2.5 rounded-xl font-bold text-sm text-black hover:opacity-90 disabled:opacity-60"
                  style={{ background: PRIMARY }}>
