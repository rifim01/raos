    ...
    <aside className="gradient-sidebar h-full flex flex-col" style={{ width: "var(--sidebar-width)" }}>
      ...
      <div className="px-6 py-5 border-b border-white/10 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-white/60 text-[10px] font-medium tracking-wider uppercase">Airport Operating System</p>
        </div>
        {onClose && (
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              <span className={cn("flex-shrink-0", isActive ? "text-white" : "text-white/60")}>
                {item.icon}
              </span>
              {item.label}
              {isActive && (
                <span className="ml-auto w-1.5 h-1.5 rounded-full bg-[#FBC02D]" />
              )}
            </Link>
          );
        })}
      ...
      <div className="px-4 py-4 border-t border-white/10">
        <p className="text-white/40 text-[10px] text-center font-medium tracking-wider">
          PT RIFIM INTERNATIONAL GEMILANG
        </p>
        <p className="text-white/20 text-[9px] text-center mt-0.5">v1.0.0 — 2025</p>
      </div>
    </aside>
  );
}
